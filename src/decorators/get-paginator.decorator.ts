...
import { PaginateConfig, Paginator } from "../paginate.index";
import { Request } from "express";

export const GetPaginator = createParamDecorator((config: PaginateConfig, ctx: ExecutionContext): Paginator => {
    ...
    const query: any = JSON.parse(queryStr);
    ...
    return new Paginator(config, query, path);
});
